...
test('create/get/list/upload/status flows', async () => { const sdk=new InsuranceSDK({apiKey:'pk_test_x',environment:'sandbox'}); const c=await sdk.claims.create(input); assert.equal(c.status,'PENDING'); assert.equal((await sdk.claims.get(c.id)).id,c.id); assert.equal((await sdk.claims.list({status:'PENDING'})).data.length,1); let progress=0; const doc=await sdk.documents.upload(c.id,{name:'receipt.pdf'},{type:'medical_receipt',onProgress:p=>progress=p}); assert.equal(doc.claimId,c.id); assert.equal(progress,100); await new Promise<void>(resolve=>sdk.claims.onStatusChange(c.id,(s)=>{assert.equal(s,'APPROVED');resolve();})); });
